...
HF_SPACE_URL = (os.environ.get("HF_SPACE_URL") or "https://1024m-hf-racing.hf.space").rstrip("/")
...
app = FastAPI(title="HF Racing Local Host")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def api_config():
    ...
    return {
        "ok": True,
        # Match WS is always same-origin (localhost). Remote Space WS was killing
        # sandbox mid-map-load. Space URL is informational for the board only.
        "spaceUrl": "",
        "boardUrl": HF_SPACE_URL,
        "username": username,
        "avatarUrl": _avatar_url() if username else None,
        "authError": err,
        "hasToken": bool(HF_TOKEN),
        "hasSpaceUrl": bool(HF_SPACE_URL),
        "lobbyHost": "local",
        "host": "local",
        "playAllowed": True,
    }
...
async def api_lobbies():
    # Local board only — Space proxy mismatched local WS and kicked sandbox mid-load.
...
async def api_claim(mode: str, lobby_id: str, body: ClaimBody):
    ...
    result = board.claim(
        ...
        username,
        body.seat,
        avatar_url=_avatar_url() or body.avatarUrl,
        car_id=body.carId,
    )
...
async def static_files(path: str = ""):
    ...
    # Stream GLBs. Allow ranges — GLTFLoader may use them; clients that need
    # a full body still work via fetch().arrayBuffer().
    if target.suffix.lower() in {".glb", ".gltf"}:
        media = "model/gltf-binary" if target.suffix.lower() == ".glb" else "model/gltf+json"
        return FileResponse(
            target,
            media_type=media,
            headers={
...
def main() -> None:
    print(f"HF Racing running at http://localhost:{PORT}")
    if HF_SPACE_URL:
